...
The 3D character lives at `web/public/models/me.glb`. **This repo ships no Blender source** — the model was generated from the author's photo with an image-to-3D service, see `tutor/intro3d处理模型教程/` for the model-prep workflow.
...
- **Media**: put images / videos in `public/works/<slug>/` and reference them from the md with `/works/...` absolute paths. Images under `public/works/` **are tracked** (only large videos are gitignored — the works detail pages reference them by absolute path, so untracking them means broken images after deploy), see `.gitignore`.
...
- Swap the 3D character: replace `public/models/me.glb` (see `tutor/intro3d处理模型教程/` for how to produce one), or rewrite `Scene.tsx` to use your own scene. The code looks these up in the glb **by object name**; whatever is missing, that feature breaks:
...
- Personal content / assets / the character model in this repo belong to **this repo's author** (Xiaoding) and are **not covered by MIT** (see `NOTICE`). Everything personal belonging to the upstream author has been removed — do not re-introduce it.
